...
| `show-file-in-submission` | boolean | true            | Whether the submission panel shows the contents of the saved file by default. Set to `false` when the submission is rendered another way (e.g. with [`pl-file-preview`](pl-file-preview.md) or [`pl-xss-safe`](pl-xss-safe.md) inside `pl-submission-panel`) to avoid displaying the file twice.                                                                                                                                                          |
...
By default, the submission panel shows the saved file's name and contents, so a student can confirm what was submitted. To show the submission a different way, set `show-file-in-submission="false"` and render it explicitly with another element such as [`pl-file-preview`](pl-file-preview.md) (which also offers a download button and previews for images, PDFs, and notebooks) or [`pl-xss-safe`](pl-xss-safe.md) (for a formatted Markdown/HTML preview) inside `pl-submission-panel`. Setting `show-file-in-submission="false"` avoids showing the file twice when one of those elements is used.
