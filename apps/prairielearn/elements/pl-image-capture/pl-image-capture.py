...
def add_format_error(answer_name: str, data: pl.QuestionData, error_string: str) -> None:
    # Record the error under both the shared "_files" key (so it appears in the
    # aggregate submission summary) and this element's answer name (so render()
    # can surface it as a per-element "Invalid" badge, matching pl-file-upload and
    # the other input elements). (PrairieLearn/PrairieLearn#14858)
...
def render(element_html: str, data: pl.QuestionData) -> str:
    ...
    # Surface this file's submission format error (e.g. a required image that was
    # left blank) as a per-element "Invalid" badge, for consistency with the other
    # input elements. parse() records these under this element's answer name via
    # add_format_error. (PrairieLearn/PrairieLearn#14858)
    file_errors = data["format_errors"].get(answer_name, [])
    parse_error = "<br>".join(file_errors) if file_errors else None
